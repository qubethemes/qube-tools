<form class="qube-tools-demo-failed-feedback-form" method="post">
    <h2><?php
        esc_html_e('Problem during demo import?', 'qube-tools')
        ?>
    </h2>
    <?php
    echo wp_nonce_field('qube_tools_demo_import_failed_feedback_form') ?>
    ...
    <p><?php esc_html_e('Clicking on send button will send your message, admin email, website url, selected demo name, php and wordpress version', 'qube-tools'); ?></p>
    <button type="submit" name="qube_tools_demo_failed_send" class="button button-secondary">
        <?php esc_html_e('Send message', 'qube-tools'); ?>
    </button>
</form>