(function ($) {
    ...
    var qubeToolsDemoImporter = {
        ...
        init: function () {
            ...
            $('.qube-tools-search-input').on('keyup', function () {
                if (0 < $(this).val().length) {
                    // Hide all items.
                    $('.qube-tools-demo-wrap .themes').find('.theme-wrap').hide();

                    // Show just the ones that have a match on the import name.
                    $('.qube-tools-demo-wrap .themes').find('.theme-wrap[data-name*="' + $(this).val().toLowerCase() + '"]').show();
                } else {
                    $('.qube-tools-demo-wrap .themes').find('.theme-wrap').show();
                }
            });

            // Prevent the popup from showing when the live preview button
            $('.qube-tools-demo-wrap .theme-actions a.button').on('click', function (e) {
                e.stopPropagation();
            });

            // Get demo data
            $('.qube-tools-open-popup').click(function (e) {
                e.preventDefault();
            ...
            $(document).on('click', 'button[name="qube_tools_demo_success_send"]', this.sendUserFeedback);
            $(document).on('click', 'button[name="qube_tools_demo_failed_send"]', this.sendUserFailedFeedback);
        ...
        categoriesFilter: function () {
            ...
            var $items = $('.qube-tools-demo-wrap .themes').find('.theme-wrap'),
                fadeoutClass = 'qube-tools-is-fadeout',
                fadeinClass = 'qube-tools-is-fadein',
            ...
            var fadeIn = function (category, dfd) {
                ...
                $items.filter(filter).show().addClass('qube-tools-is-fadein');
            ...
            $('.qube-tools-navigation-link').on('click', function (event) {
                event.preventDefault();
                ...
                var $container = $('.qube-tools-demo-wrap .themes');
        ...
        getDemoData: function (demo_name) {
            ...
            $.ajax({
                ...
                data: {
                    action: 'qube_tools_ajax_get_import_data',
                    demo_name: demo_name,
                    security: qubeToolsDemos.qube_tools_import_data_nonce
                },
            ...
            $.ajax({
                ...
                data: {
                    action: 'qube_tools_ajax_get_demo_data',
                    demo_name: demo_name,
                    demo_data_nonce: qubeToolsDemos.demo_data_nonce
                },
        ...
        runPopup: function (data) {
            ...
            // Show popup
            $('#qube-tools-demo-popup-wrap').fadeIn();
            $(data.responseText).appendTo($('#qube-tools-demo-popup-content'));

            // Close popup
            $('.qube-tools-demo-popup-close, .qube-tools-demo-popup-overlay').on('click', function (e) {
                e.preventDefault();
            ...
            $('.qube-tools-plugins-next').on('click', function (e) {
                e.preventDefault();

                // Hide step one
                $('#qube-tools-demo-plugins').hide();

                // Display step two
                $('#qube-tools-demo-import-form').show();

            });

            // if clicked on import data button
            $('#qube-tools-demo-import-form').submit(function (e) {
                e.preventDefault();

                // Vars
                var demo = $(this).find('[name="qube_tools_import_demo"]').val(),
                    nonce = $(this).find('[name="qube_tools_import_demo_data_nonce"]').val(),
                    contentToImport = [];
                ...
                that.importContent({
                    ...
                    isXML: $('#qube_tools_import_xml').is(':checked')
                });
            });
        ...
        importContent: function (importData) {
            var that = this,
                ...
                timerStart = Date.now(),
                ajaxData = {
                    qube_tools_import_demo: importData.demo,
                    qube_tools_import_demo_data_nonce: importData.nonce
                };

            this.allowPopupClosing = false;
            $('.qube-tools-demo-popup-close').fadeOut();
            ...
            if (importData.contentToImport.length === 0) {
                ...
                setTimeout(function () {
                    $('.qube-tools-loader').hide();
                    $('.qube-tools-last').show();
                }, 1000);
                ...
                $.ajax({
                    ...
                    type: 'post',
                    data: {
                        action: 'qube_tools_after_import',
                        qube_tools_import_demo: importData.demo,
                        qube_tools_import_demo_data_nonce: importData.nonce,
                        qube_tools_import_is_xml: importData.isXML
                    },
                    complete: function (data) {
                    }
                });
                ...
                $('.qube-tools-demo-popup-close').fadeIn();
            ...
            $('.qube-tools-import-status').append('<p class="qube-tools-importing">' + this.importData[currentContent]['loader'] + '</p>');
            ...
            var ajaxRequest = $.ajax({
                ...
                complete: function (data) {
                    ...
                    if (data.status === 500 || data.status === 502 || data.status === 503) {
                        $('.qube-tools-importing')
                            .addClass('qube-tools-importing-failed')
                            .removeClass('qube-tools-importing')
                            .text(qubeToolsDemos.content_importing_error + ' ' + data.status);
                        $('.qube-tools-demo-failed-feedback-form').find('.error_message').val(qubeToolsDemos.content_importing_error + ' ' + data.status);
                        $('.qube-tools-demo-failed-feedback-form').show();

                    } else if (data.responseText.indexOf('successful import') !== -1) {
                        $('.qube-tools-importing').addClass('qube-tools-imported').removeClass('qube-tools-importing');
                    } else {
                        ...
                        $('.qube-tools-importing')
                            ...
                            .removeClass('qube-tools-importing')
                            .text(errorMessage);
                        $('.qube-tools-demo-failed-feedback-form').find('.error_message').val(errorMessage);
                        $('.qube-tools-demo-failed-feedback-form').show();
            ...
            importingLimit = setTimeout(function () {
                ...
                $('.qube-tools-importing')
                    .addClass('qube-tools-importing-failed')
                    .removeClass('qube-tools-importing')
                    .text(qubeToolsDemos.content_importing_error);
                $('.qube-tools-demo-failed-feedback-form').find('.error_message').val(qubeToolsDemos.content_importing_error);
                $('.qube-tools-demo-failed-feedback-form').show();
        ...
        closePopup: function () {
            ...
            // Hide demo popup
            $('#qube-tools-demo-popup-wrap').fadeOut();

            // Remove content in the popup
            setTimeout(function () {
                $('#qube-tools-demo-popup-content').html('');
            }, 600);
        },
        ...
        activatePlugins: function (e) {
            ...
            $button.closest('.qube-tools-plugin').find('h2').find('.plugin-activated-icon').remove();
            $button.addClass('updating-message button-primary').html(qubeToolsDemos.button_activating);

            $.ajax({
                url: qubeToolsDemos.ajaxurl,
                type: 'POST',
                data: {
                    action: 'qube_tools_ajax_required_plugins_activate',
                    init: $init,
                },
                ...
                if (result.success) {
                    ...
                    $button.removeClass('button-primary install-now activate-now updating-message')
                        ...
                        .addClass('disabled')
                        .text(qubeToolsDemos.button_activated);
                    $button.closest('.qube-tools-plugin').find('h2').prepend('<span class="plugin-activated-icon dashicons dashicons-yes-alt"></span>')
        ...
        installSuccess: function (e, response) {
            ...
            var $message = $('.qube-tools-plugin-' + response.slug).find('.button');
            $message.closest('.qube-tools-plugin').find('h2').find('.plugin-activated-icon').remove();
            ...
            $message.removeClass('install-now installed button-disabled updated-message')
                ...
                .html(qubeToolsDemos.button_activating);
                ...
                $.ajax({
                    ...
                    if (result.success) {
                        ...
                        $message.removeClass('button-primary install-now activate-now updating-message')
                            ...
                            .addClass('disabled')
                            .text(qubeToolsDemos.button_activated);
                        $message.closest('.qube-tools-plugin').find('h2').prepend('<span class="plugin-activated-icon dashicons dashicons-yes-alt"></span>')
        ...
        installError: function (e, response) {
            ...
            var $card = $('.qube-tools-plugin-' + response.slug);
